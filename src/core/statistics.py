...
class StatisticsCore:
    ...
    def list_ranking_sellers(self, data: dict):
        try:
            ...
            list_rank_tables = self.pg.fetch_to_dict(query=self.models.list_ranking_sellers(pagination=pagination))
            if not list_rank_tables:
                return Response().response(status_code=404, error=True, message_id="list_ranking_sellers_list_not_found", exception="Not found", data=list_rank_tables)
            ...
            return Response().response(status_code=200, message_id="list_ranking_sellers_successful", data=list_rank_tables, metadata=metadata)
