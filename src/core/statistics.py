                ...
                select(
                    ...
                    func.to_char(
                        func.round(
                            func.sum(
                                func.coalesce(
                                    self.proposal_loan.valor_operacao, 0
                                )
                            ).cast(Numeric),
                            2,
                        ),
        ...
        try:
            ...
            stmt = (
                select(
                    ...
                    .over(
                        order_by=func.sum(
                            func.coalesce(pl.valor_operacao, 0)
                        ).desc()
                    )
                    .label("posicao"),
                    ...
                    func.to_char(
                        func.round(
                            func.sum(func.coalesce(pl.valor_operacao, 0)).cast(
                                Numeric
                            ),
                            2,
                        ),
                ...
                .group_by(func.upper(u.username))
            ...
            logdb(
                "error", message=f"Error processing list_ranking_sellers. {e}"
            )
            return Response().response(
                status_code=500,
                error=True,
