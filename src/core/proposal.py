...
class ProposalCore:
    ...
    def list_proposal(self, data: dict):
        try:
            ...
            contract_paid_cte = (
                select(
                    ...
                    case(
                        (
                            ...
                            self.proposal_status.action_at,
                        ),
                        (
                    ...
                    case(
                        (
                            self.proposal_status.contrato_pago,
                            self.proposal_status.action_by,
                        ),
                        (
                            self.proposal_status.aguardando_digitacao,
                            self.proposal_status.action_by,
                ...
                .select_from(self.proposal_status)
                .outerjoin(
                    u_alias, u_alias.id == self.proposal_status.action_by
                )
                .cte("contract_paid_cte")
            )
            ...
            stmt = (
                select(
                    ...
                    func.to_char(
                        self.proposal.created_at, "YYYY-MM-DD HH24:MI"
                    ).label("created_at"),
                    self.proposal.cpf,
                    ...
                    func.to_char(
                        self.proposal.updated_at, "YYYY-MM-DD HH24:MI"
                    ).label("updated_at"),
                    func.initcap(func.trim(cp.c.status_updated_by_name)).label(
                ...
                .outerjoin(cp, cp.c.proposal_id == self.proposal.id)
                .where(
            ...
            if pagination["filter_by"]:
                ...
                stmt = stmt.where(
                    or_(
                        ...
                        func.unaccent(cp.c.current_status).ilike(
                            func.unaccent(filter_value)
                        ),
                    )
                )
            ...
            if pagination["order_by"]:
                sort_column = getattr(
                    self.proposal, pagination["order_by"], None
                )
    ...
    def add_proposal(self, data: dict, image_data: FileStorage):
        try:
            ...
            new_data = {
                k: v for k, v in data_dict.items() if v
            ...
            proposal_columns = {
                col.name for col in self.proposal.__table__.columns
            }
            wallet_columns = {
                col.name for col in self.proposal_wallet.__table__.columns
            }
            loan_columns = {
                col.name for col in self.proposal_loan.__table__.columns
            }
            benefit_columns = {
                col.name for col in self.proposal_benefit.__table__.columns
            }

            # Separando os dados para cada tabela
            proposal_data = {
                k: v for k, v in new_data.items() if k in proposal_columns
            }
            wallet_data = {
                k: v for k, v in new_data.items() if k in wallet_columns
            }
            loan_data = {
                k: v for k, v in new_data.items() if k in loan_columns
            }
            benefit_data = {
                k: v for k, v in new_data.items() if k in benefit_columns
            }

            # Inserir proposta principal
            proposal_stmt = (
                insert(self.proposal)
                .values(
                    **proposal_data, user_id=self.user_id, is_deleted=False
                )
                .returning(self.proposal.id)
            )
            ...
            if proposal_id:
                ...
                if new_data.get("benefit_id"):
                    benefit_stmt = insert(self.proposal_benefit).values(
                        proposal_id=proposal_id, **benefit_data
                    )
        ...
        try:
            ...
            stmt = (
                select(
                    ...
                    func.initcap(func.trim(User.username)).label(
                        "name_seller"
                    ),
                    self.proposal.id,
                    ...
                    func.to_char(
                        self.proposal.data_emissao, "DD-MM-YYYY HH24:MI"
                    ).label("data_emissao"),
                    ...
                    func.initcap(func.trim(self.proposal.nome_mae)).label(
                        "nome_mae"
                    ),
                    func.initcap(func.trim(self.proposal.nome_pai)).label(
                        "nome_pai"
                    ),
                    func.initcap(func.trim(self.proposal.bairro)).label(
                        "bairro"
                    ),
                    func.initcap(func.trim(self.proposal.endereco)).label(
                        "endereco"
                    ),
                    self.proposal.numero_endereco,
                    self.proposal.complemento_endereco,
                    func.initcap(func.trim(self.proposal.cidade)).label(
                        "cidade"
                    ),
                    ...
                    self.proposal.cep,
                    func.to_char(
                        self.proposal.data_nascimento, "DD-MM-YYYY HH24:MI"
                    ...
                    func.to_char(
                        self.proposal_loan.data_dispacho, "DD-MM-YYYY HH24:MI"
                    ).label("data_dispacho"),
                    ...
                    ).label("name_financial_agreements"),
                    ...
                    func.initcap(
                        func.trim(self.proposal_wallet.tipo_conta)
                    ).label("tipo_conta"),
                    self.proposal_wallet.tipo_pagamento,
                    self.benefit.id.label("id_beneficio"),
                    func.initcap(func.trim(self.benefit.name)).label(
                        "tipo_beneficio"
                    ),
                )
                ...
                .outerjoin(
                    self.bankers,
                    self.financial_agreements.banker_id == self.bankers.id,
                )
                .outerjoin(
                    self.proposal_wallet,
                    self.proposal_wallet.proposal_id == self.proposal.id,
                )
                .outerjoin(
                    self.proposal_benefit,
                    self.proposal_benefit.proposal_id == self.proposal.id,
                )
                .outerjoin(
                    self.benefit,
                    self.proposal_benefit.benefit_id == self.benefit.id,
                )
                .outerjoin(
                    self.tables,
                    self.proposal_loan.tables_finance_id == self.tables.id,
                )
                .where(
                    self.proposal.id == id, self.proposal.is_deleted == False
                )  # Corrigido
            )
            ...
            for field in FIELDS_WITH_IMAGES:
                ...
                if os.path.exists(field_directory) and os.path.isdir(
                    field_directory
                ):
                    files = [
                        file
                        for file in os.listdir(field_directory)
    ...
    def update_proposal(
        ...
        proposal_id: int,
        data: dict,
        image: dict[str, list[FileStorage]] | None,
    ):
        try:
            ...
            for table_name, fields in tables_and_fields.items():
                ...
                if table_name == "proposal":
                    ...
                    if "benefit_id" in data:
                        benefit_id = self.convert_value(
                            data["benefit_id"], int
                        )
                        if benefit_id:
                            benefit = (
                                db.session.query(self.benefit)
                                .filter(self.benefit.id == benefit_id)
                                .first()
                            )
                            if not benefit:
                                return Response().response(
                            ...
                            existing = (
                                db.session.query(self.proposal_benefit)
                                .filter(
                                    self.proposal_benefit.proposal_id
                                    == proposal_id,
                                    self.proposal_benefit.benefit_id
                                    == benefit_id,
                    ...
                    instance = (
                        ...
                        .filter(getattr(model, "proposal_id") == proposal_id)
                        .first()
                    )
                    if not instance:
            ...
            if image_data:
                image_data = {
                    ...
                    if files and all(file.filename for file in files)
                }
                uploader = UploadProposal(
                    proposal_id=proposal_id,
                    user_id=self.user_id,
    ...
    def delete_proposal(self, id: int):
        try:
            ...
            # soft deleted
            self.proposal.query.filter_by(id=id).update(
                {
                    "is_deleted": True,
            ...
            self.proposal_benefit.query.filter_by(id=id).update(
                {
                    "is_deleted": True,
                    "deleted_by": self.user_id,
                    "deleted_at": datetime.now(),
            ...
            self.proposal_loan.query.filter_by(id=id).update(
                {
                    "is_deleted": True,
                    "deleted_by": self.user_id,
                    "deleted_at": datetime.now(),
            ...
            self.proposal_status.query.filter_by(id=id).update(
                {
                    "is_deleted": True,
                    "deleted_by": self.user_id,
                    "deleted_at": datetime.now(),
            ...
            self.proposal_wallet.query.filter_by(id=id).update(
                {
                    "is_deleted": True,
                    "deleted_by": self.user_id,
                    "deleted_at": datetime.now(),
            ...
            self.manage_operational.query.filter_by(id=id).update(
                {
                    "is_deleted": True,
                    "deleted_by": self.user_id,
                    "deleted_at": datetime.now(),
