...
class RoleCore:
    ...
    def list_role(self, data: dict):
        ...
        stmt = select(
            self.role.id, func.initcap(func.trim(self.role.name)).label("name")
        ...
        if pagination["filter_value"]:
            ...
            query = query.filter(
                db.or_(
                    func.unaccent(self.role.name).ilike(
        ...
        total = db.session.execute(
            select(func.count(self.role.id)).where(
                self.role.is_deleted == False
            )
        ).scalar()
    ...
    def add_role(self, data: dict):
        try:
            ...
            if isinstance(e.orig, UniqueViolation):
                ...
                return Response().response(
                    status_code=400, message_id="role_name_already_exists"
                )
            else:
                self.role.query.session.rollback()
            ...
            return Response().response(
                status_code=500, message_id="role_error", exception=str(e)
            )

    def delete_role(self, id: int):
        try:
            role = self.role.query.filter_by(id=id).update(
                {"is_deleted": True}
            )
            if not role:
                return Response().response(
            ...
            self.role.query.session.commit()
            return Response().response(
                status_code=200, message_id="role_delete_successful"
            )
        except Exception as e:
            self.role.query.session.rollback()
