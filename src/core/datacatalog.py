...
class DataCatalogBenefitCore:
    ...
    def add_benefit(self, data: dict):
        try:
            ...
            benefit = self.benefit(name=name, created_at=datetime.now())
    ...
    def list_benefit(self, data: dict):
        try:
            ...
            stmt = select(self.benefit.id, self.benefit.name).where(
                self.benefit.is_deleted == False
            )

            # Ordenação dinâmica
            if pagination["order_by"] and pagination["sort_by"]:
                sort_column = getattr(
                    self.benefit, pagination["order_by"], None
                )
                if sort_column:
            ...
            total = db.session.execute(
                select(func.count(self.benefit.id)).where(
                    self.benefit.is_deleted == False
                )
            ).scalar()
...
class DataCatalogBankCore:
    ...
    def add_bank(self, data: dict):
        try:
            ...
            bank = self.bank(name=name, id_bank=id_bank)
            db.session.add(bank)
            db.session.commit()

            return Response().response(
                status_code=200, message_id="bank_add_successful", error=False
            )
    ...
    def list_bank(self, data: dict):
        try:
            ...
            if pagination["filter_value"]:
                ...
                stmt = stmt.where(
                    or_(
                        func.unaccent(self.bank.name).ilike(
            ...
            total = db.session.execute(
                select(func.count(self.bank.id)).where(
                    self.bank.is_deleted == False
                )
            ).scalar()
...
class DataCatalogLoanOperationCore:
    ...
    def list_loan_operation(self, data: dict):
        try:
            ...
            if pagination["filter_value"]:
                ...
                stmt = stmt.where(
                    or_(
                        func.unaccent(self.loan_operation.name).ilike(
                            func.unaccent(filter_value)
                        )
                    )
                )
            ...
            if pagination["sort_by"]:
                sort_column = getattr(
                    self.loan_operation, pagination["sort_by"]
                )
                if pagination["order_by"] == "asc":
                    stmt = stmt.order_by(sort_column.asc())
                else:
            ...
            total = db.session.execute(
                select(func.count(self.loan_operation.id)).where(
    ...
    def delete_loan_operation(self, id: int):
        try:
            ...
            return Response().response(
                status_code=200,
                message_id="loan_operation_delete_successful",
                error=False,
            )
