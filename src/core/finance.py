...
from src.models.models import Bankers, FinancialAgreements, ObtianReport, TablesFinance, User
...
class BankersCore:
    ...
    def list_bankers(self, data: dict) -> None:
        try:
            ...
            stmt = select(
                self.bankers.id,
                self.bankers.name,
                func.to_char(self.bankers.created_at, 'YYYY-MM-DD').label('created_at')
            ).where(
            ...
            if pagination["filter_value"]:
                ...
                stmt = stmt.where(
                    or_(
                        func.unaccent(self.bankers.name).ilike(func.unaccent(filter_value))
                    )
                )

            if pagination["order_by"]:
                sort_column = getattr(self.bankers, pagination["order_by"], None)
            ...
            count_stmt = select(func.count()).select_from(
                ...
                .where(
                    or_(
                        func.unaccent(self.bankers.name).ilike(func.unaccent(filter_value))
                    ) if pagination["filter_value"] else True
                ).subquery()
            )
    ...
    def get_banker(self, id: int) -> None:
        try:
            stmt = select(
                ...
                func.initcap(func.trim(self.bankers.name)).label('bank_name'),
                func.to_char(self.bankers.created_at, 'YYYY-MM-DD').label('created_at'),
                func.json_agg(
                    func.json_build_object(
                        'id_financialagreements', self.financial_agreements.id,
                        'financialagreements_name', func.initcap(func.trim(self.financial_agreements.name))
                    )
            ...
            return Response().response(status_code=500, error=True, message_id="banker_get_error")
    ...
    def add_banker(self, data: dict) -> None:
        try:
            ...
            bankers = insert(self.bankers).values(
                name=data.get("name"),
    ...
    def update_banker(self, id: int, data: dict) -> None:
        try:
            ...
            bankers = update(self.bankers).where(self.bankers.id == id).values(
                name=data.get("name"),
            ...
            return Response().response(
                status_code=500,
                error=True, 
                message_id="bankers_update_error"
            )
    ...
    def delete_banker(self, id: int) -> None:
        try:
            ...
            bankers = update(self.bankers).where(self.bankers.id == id).values(is_deleted=True).returning(self.bankers.id)
            bankers = db.session.execute(bankers).scalar()
            db.session.commit()
...
class FinancialAgreementsCore:
    ...
    def add_financial_agreements(self, data: dict) -> None:
        try:
            ...
            financial_agreements = insert(self.financial_agreements).values(
                name=data.get("name"),
                banker_id=data.get("banker_id"),
                created_at= datetime.now()
            ).returning(self.financial_agreements.id)
            financial_agreements = db.session.execute(financial_agreements).scalar()
            db.session.commit()
    ...
    def update_financial_agreements(self, id: int, data: dict) -> None:
        try:
            ...
            financial_agreements = update(self.financial_agreements).where(self.financial_agreements.id == id).values(
                name=data.get("name"),
                updated_at= datetime.now(),
                updated_by=self.user_id 
            ).returning(self.financial_agreements.id)
    ...
    def delete_financial_agreements(self, id: int) -> None: 
        try:
            ...
            financial_agreements = update(self.financial_agreements).where(
                self.financial_agreements.id == id
                ).values(
                    is_deleted=True, 
...
class TablesCore:
    ...
    def add_tables_import(self, data: dict, file: FileStorage) -> None:
        ...
        try:
            ...
            # Leitura do arquivo
            dftmp = read_excel(filepath, dtype="object", engine="openpyxl").fillna("")

            if dftmp.empty:
                return Response().response(
                    status_code=400,
                    error=True,
                    message_id="empty_excel_file"
                )

            required_columns = [
                'Tabela', 'Tipo', 'Cod Tabela', 'Prazo Inicio', 
                'Prazo Fim', 'Flat', 'Taxa Inicio', 'Taxa Fim'
            ]
    ...
    def list_board_table(self, data: dict, financial_agreements_id: int) -> None:
        try:
            ...
            stmt = select (
                ...
                self.tables.is_deleted.is_(False),
                self.tables.financial_agreements_id == financial_agreements_id
            ).group_by(
                self.tables.id,
            )
...
class ReportCore:
    ...
    def add_report(self, data: dict, file: FileStorage) -> None:
        try:
            required_columns = ['CPF', 'NOME', 'FLAT', 'BONUS', 'NUMERO_PROPOSTA', 'VALOR_OPERACAO']
            ...
            if not all(required_columns in dftmp.columns for required_columns in required_columns):
                logdb("warning", message=f"Missing required columns. {data.get('name')}")
                return Response().response(
                    status_code=400,
                    error=True,
    ...
    def list_report_imports(self, data: dict) -> None:
        try:
            ...
            stmt = select(
                ...
                func.to_char(self.report.created_at, 'YYYY-MM-DD').label('created_at')
            ).distinct(self.report.name_report).join(
                self.user, self.report.user_id == self.user.id
            ).where(
                self.report.is_deleted == False,
                self.user.is_deleted == False
            ).order_by(
                self.report.name_report,
                self.report.created_at.desc()
            )
            ...
            if pagination["order_by"]:
                sort_column = getattr(self.report, pagination["order_by"], None)
                if sort_column is not None:
                    if pagination["sort_by"] == "asc":
                        stmt = stmt.order_by(sort_column.asc())
                    else:
            ...
            count_stmt = select(func.count()).select_from(
                ...
                .where(
                    or_(
                        func.unaccent(self.report.name_report).ilike(func.unaccent(filter_value))
                    ) if pagination["filter_value"] else True
                ).subquery()
            )
    ...
    def delete_imports(self, name: str) -> None:
        try:
            stmt = update(self.report).where(self.report.name_report == name).values(
                is_deleted=True, deleted_at=datetime.now(), deleted_by=self.user_id
            )
            db.session.execute(stmt)
            db.session.commit()
            return Response().response(
