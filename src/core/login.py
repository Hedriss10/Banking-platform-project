    ...
    def get_login(self, data: dict):
        ...
        if not user or not user.id:
            return Response().response(
                message_id="user_not_found", status_code=404, error=True
            )
        ...
        stmt = (
            select(
                ...
                func.to_char(self.user.create_at, "YYYY-MM-DD").label(
                    "create_at"
                ),
            )
        ...
        if not password:
            return Response().response(
                message_id="password_not_user", status_code=400, error=True
            )
        ...
        if is_valid:
            ...
            return Response().response(
                message_id="incorrect_password", status_code=401, error=True
            )

    def reset_password_authorization(self, data: dict):
        try:
            new_password = generate_password_hash(
                password="123@bs", method="scrypt"
            )
            ...
            return Response().response(
                message_id="resert_password_error", status_code=401, error=True
            )
