...
@tables_finance_ns.route("/list-bankers-agreements")
class ListBankersFinancialAgreementsResource(Resource):
    
    # @jwt_required()
    @tables_finance_ns.doc(description="List of ids related to bankers and financialagreements")
    @tables_finance_ns.expect(pagination_arguments_customer, validate=True)
    @cross_origin()
    def get(self):
        """List of ids related to bankers and financialagreements""" 
        try:
            user_id = request.headers.get("Id", request.environ.get("Id"))      
            
            return TablesFinanceCore(user_id=user_id).list_bankers_financial_agreements(data=request.args.to_dict())
        except Exception as e:
            return Response().response(status_code=400, error=True, message_id="something_went_wrong", exception=str(e), traceback=traceback.format_exc(e))
...
@tables_finance_ns.route("/board-table/<int:id>/<int:banker_id>/<int:financial_agreements_id>")  
class DeleteTableResource(Resource):
    
    # @jwt_required()
    @tables_finance_ns.doc(description="Delete tables banker_id and financialagreements_id")
    @cross_origin()
    def delete(self, id, banker_id, financial_agreements_id):
        """Delete tables with board filter by in banker_id with financial_agreements"""
        try:
            user_id = request.headers.get("Id", request.environ.get("Id"))      
            
            return TablesFinanceCore(user_id=user_id).delete_tables(id=id, banker_id=banker_id, financial_agreements_id=financial_agreements_id)
        except Exception as e:
            return Response().response(status_code=400, error=True, message_id="something_went_wrong", exception=str(e), traceback=traceback.format_exc(e))
...
class DeleteTablesIdsResource(Resource):
    ...
    @cross_origin()
    def post(self, banker_id, financial_agreements_id):
