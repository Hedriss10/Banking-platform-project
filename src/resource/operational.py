...
payload_operational = operatinal_ns.model(
    "TypingProposal",
    {
        ...
        "aceite_feito_analise_banco": fields.Boolean(required=False, example=False),
        "contrato_pendente_banco": fields.Boolean(required=False, example=False),
        "aguardando_pagamento": fields.Boolean(required=False, example=False),
        "contrato_pago": fields.Boolean(required=False, example=True),
        "description": fields.String(required=False, example="Description of proposal"),
...
class ListProposalResource(Resource):
    ...
    def post(self, proposal_id: int):
        ...
        try:
            ...
            return OperationalCore(user_id=user_id).typing_proposal(proposal_id=proposal_id, data=request.get_json())
...
class ListProposalResource(Resource):
    ...
    def get(self):
        ...
        try:
            ...
            return OperationalCore(user_id=user_id).list_proposal(data=request.args.to_dict())
        except Exception as e:
            return Response().response(status_code=500, error=True, message_id="something_went_wrong", exception=str(e), traceback=traceback.format_exc())
...
class ListProposalHistoryResource(Resource):
    ...
    def get(self, id):
        ...
        try:
            ...
            return OperationalCore(user_id=user_id).history_proposal(proposal_id=id, data=request.args.to_dict())
        except Exception as e:
            return Response().response(status_code=500, error=True, message_id="something_went_wrong", exception=str(e), traceback=traceback.format_exc())
